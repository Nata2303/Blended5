const refs = {
  modalTable: document.querySelectorAll('[data-modal-table]'),
  backdrop: document.querySelector('[data-backdrop]'),
};

const markupTable = ` <div class="modal">
...
refs.modalTable.forEach(el => {
  el.addEventListener('click', openTableModal);
});

function openTableModal(event) {
  refs.backdrop.classList.toggle('activ');
  refs.backdrop.innerHTML += markupTable;

  const closeModalTable = document.querySelector('[data-close-table]');

  closeModalTable.addEventListener('click', closeTable);

  function closeTable(evt) {
    refs.backdrop.classList.toggle('activ');
